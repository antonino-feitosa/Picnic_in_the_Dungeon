...
class ActionButton(ControlComponent):
    def __init__(
        self, game: Game, position: Point, callback: Callable[[bool], None]
    ):
        super().__init__(game[ControlSystem])
    ...
    def mouseLeft(self, down: bool, screenPosition: Point, worldPosition: Point) -> bool:
...
def createMainScreenGame(gameLoop: GameLoop) -> Game:
    ...
    def loadGame(screenPosition: Point, worldPosition: Point) -> bool:
        sound.stop()
    ...
    forceTransition.callback = lambda: loadGame(Point(), Point())  # type: ignore
